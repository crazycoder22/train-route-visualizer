...
function getStatusColor(status: string, confirmed: boolean): string {
  ...
  if (upper.startsWith("WL") || upper.startsWith("RLWL") || upper.startsWith("GNWL") || upper.startsWith("PQWL"))
    return "text-red-600";
  if (upper.startsWith("CAN")) return "text-slate-500";
...
function getStatusBg(status: string, confirmed: boolean): string {
  ...
  const upper = status.toUpperCase();
  if (upper.startsWith("RAC")) return "bg-amber-50 border-amber-200";
  if (upper.startsWith("WL") || upper.startsWith("RLWL") || upper.startsWith("GNWL") || upper.startsWith("PQWL"))
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        ...
        <p className="text-sm text-slate-500 mb-4">Enter your 10-digit PNR number to check booking status</p>
        ...
        <form onSubmit={handleSubmit} className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none text-slate-800 placeholder:text-slate-400 transition-all font-mono text-lg tracking-wider"
            maxLength={10}
            inputMode="numeric"
          />
          <button
      ...
      {loading && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-slate-500 text-sm">Checking PNR status...</p>
          </div>
        </div>
      )}
      ...
      {data && (
        <>
          ...
          >
            ...
            {data.chartPrepared && (
              <div className="mt-2 text-xs font-medium text-slate-600 bg-white/60 rounded-lg px-2 py-1 w-fit">
                Chart Prepared
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-50 rounded-xl p-3 sm:col-span-2">
                ...
                {data.bookingDate && (
                  <div className="text-xs text-slate-500">Booked: {data.bookingDate}</div>
                )}
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-3 sm:col-span-2">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-center">
                    <div className="text-sm font-bold text-slate-800">{data.boardingPointName || data.fromName}</div>
                    <div className="text-[10px] text-slate-400 font-mono">{data.boardingPoint || data.from}</div>
                    {data.departureTime && (
                  ...
                  <div className="text-center">
                    <div className="text-sm font-bold text-slate-800">{data.reservationUptoName || data.toName}</div>
                    <div className="text-[10px] text-slate-400 font-mono">{data.reservationUpto || data.to}</div>
                    {data.arrivalTime && (
              ...
              <div className="bg-slate-50 rounded-xl p-3">
                <div className="text-[11px] text-slate-400 uppercase tracking-wider">Class</div>
                <div className="text-sm font-semibold text-slate-800">{data.travelClass || "—"}</div>
              </div>

              <div className="bg-slate-50 rounded-xl p-3">
                <div className="text-[11px] text-slate-400 uppercase tracking-wider">Quota</div>
                <div className="text-sm font-semibold text-slate-800">{data.quota || "—"}</div>
              </div>
              ...
              {data.bookingFare && (
                <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <div className="text-sm font-semibold text-slate-800">Rs. {data.bookingFare}</div>
                </div>
              )}
              ...
              {data.expectedPlatform && (
                <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <div className="text-sm font-semibold text-slate-800">{data.expectedPlatform}</div>
                </div>
              )}
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-slate-700">Passenger {p.number}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-slate-400 text-xs">Booking Status</span>
                        <div className="font-mono font-medium text-slate-600">{p.bookingStatus || "—"}</div>
                      </div>
                      <div>
                      ...
                      {p.confirmed && p.coach && (
                        <>
                          <div>
                            <span className="text-slate-400 text-xs">Coach / Berth</span>
                          ...
                          {p.berthType && (
                            <div>
                              <span className="text-slate-400 text-xs">Berth Type</span>
      ...
      {!data && !loading && !error && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-slate-700 mb-1">Check Your PNR Status</h3>
